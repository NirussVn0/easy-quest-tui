...
import type { AllQuestsResponse, QuestTaskConfigType, Quest as QuestShape } from '../types/quest.types';
...
export class QuestManager implements Iterable<Quest> {
	...
	async doingQuest(quest: Quest) {
		...
		const applicationName = quest.config?.application?.name || 'Unknown Application';
		...
		if (
            ...
            return 'Completed';
		} else if (taskName === 'PLAY_ON_DESKTOP' || taskName === 'STREAM_ON_DESKTOP' || taskName === 'PLAY_ACTIVITY') {
