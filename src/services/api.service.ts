...
import { GatewaySendPayload, GatewayOpcodes } from 'discord-api-types/v9';
import { QuestManager } from './quest.service';
import { AllQuestsResponse } from '../types/quest.types';
...
async function makeRequest(url: string, init: RequestInit): Promise<ResponseLike> {
  if (init.headers) {
    ...
    if (myHeaders.has('Authorization')) {
      myHeaders.set('Authorization', myHeaders.get('Authorization')!.replace('Bot ', ''));
    }
    ...
    myHeaders.append(
      'x-super-properties',
      Buffer.from(JSON.stringify(Constants.Properties)).toString('base64'),
    );
...
export class ClientQuest extends Client {
  ...
  fetchQuests() {
    return this.rest.get('/quests/@me').then((response) => {
      this.questManager = QuestManager.fromResponse(this, response as AllQuestsResponse);
